...
void ofApp::update(){
    ...
    Bcolor = ofColor(ofMap(sin(angle), 0, 1, 0, 100));
...
void ofApp::CheckBlackHole(){
    for(int i=0;i<movers.size();i++){
        for(int ii=0;ii<BlackHoles.size();ii++){
            if(movers[i].pos.x > 0 && movers[i].pos.x < ofGetWidth() && movers[i].pos.y < ofGetHeight() && movers[i].pos.y > 0){
